...
install_base(){
  ...
  if ! command -v jq &> /dev/null; then
      echo "jq is not installed. Please install jq manually and rerun the script."
      exit 1
  fi
}
...
regenarte_cloudflared_argo(){
  ...
  vmess_port=$(jq -r '.inbounds[2].listen_port' ./sbox/sbconfig_server.json)
  #生成地址
  ./sbox/cloudflared-linux tunnel --url http://localhost:$vmess_port --no-autoupdate --edge-ip-version auto --protocol h2mux > argo.log 2>&1 &
  sleep 2
  clear
  ...
  echo "$argo" | base64 > ./sbox/argo.txt.b64
  rm -rf argo.log
...
download_singbox(){
  ...
  curl -sLo "./${package_name}.tar.gz" "$url"

  # Extract the package and move the binary to ./sbox
  tar -xzf "./${package_name}.tar.gz" -C ./
  mv "./${package_name}/sing-box" ./sbox

  # Cleanup the package
  rm -r "./${package_name}.tar.gz" "./${package_name}"

  # Set the permissions
  chmod +x ./sbox/sing-box
}
...
download_cloudflared(){
  ...
  curl -sLo "./sbox/cloudflared-linux" "$cf_url"
  chmod +x ./sbox/cloudflared-linux
  echo ""
}
...
show_client_configuration() {
  ...
  current_listen_port=$(jq -r '.inbounds[0].listen_port' ./sbox/sbconfig_server.json)
  # Get current server name
  current_server_name=$(jq -r '.inbounds[0].tls.server_name' ./sbox/sbconfig_server.json)
  # Get the UUID
  uuid=$(jq -r '.inbounds[0].users[0].uuid' ./sbox/sbconfig_server.json)
  # Get the public key from the file, decoding it from base64
  public_key=$(base64 --decode ./sbox/public.key.b64)
  # Get the short ID
  short_id=$(jq -r '.inbounds[0].tls.reality.short_id[0]' ./sbox/sbconfig_server.json)
  ...
  echo ""
  # Get current listen port
  hy_current_listen_port=$(jq -r '.inbounds[1].listen_port' ./sbox/sbconfig_server.json)
  # Get current server name
  hy_current_server_name=$(openssl x509 -in ./self-cert/cert.pem -noout -subject -nameopt RFC2253 | awk -F'=' '{print $NF}')
  # Get the password
  hy_password=$(jq -r '.inbounds[1].users[0].password' ./sbox/sbconfig_server.json)
  # Generate the link
...
EOF
  ...
  argo=$(base64 --decode ./sbox/argo.txt.b64)
  vmess_uuid=$(jq -r '.inbounds[2].users[0].uuid' ./sbox/sbconfig_server.json)
  ws_path=$(jq -r '.inbounds[2].transport.path' ./sbox/sbconfig_server.json)
...
uninstall_singbox() {
          ...
          # Remove files
          rm ./sbox/sbconfig_server.json
          rm ./sbox/sing-box
          rm ./sbox/cloudflared-linux
          rm ./sbox/argo.txt.b64
          rm ./sbox/public.key.b64
          rm ./self-cert/private.key
          rm ./self-cert/cert.pem
          rm -rf ./self-cert/
          rm -rf ./sbox/
          echo "DONE!"
}
...
if [ -f "./sbox/sbconfig_server.json" ] && [ -f "./sbox/sing-box" ] && [ -f "./sbox/public.key.b64" ] && [ -f "./sbox/argo.txt.b64" ]; then
    ...
    case $choice in
        ...
        2)
          ...
          jq --arg listen_port "$listen_port" --arg server_name "$server_name" --arg hy_listen_port "$hy_listen_port" '.inbounds[1].listen_port = ($hy_listen_port | tonumber) | .inbounds[0].listen_port = ($listen_port | tonumber) | .inbounds[0].tls.server_name = $server_name | .inbounds[0].tls.reality.handshake.server = $server_name' ./sbox/sbconfig_server.json > ./sb_modified.json
          mv ./sb_modified.json ./sbox/sbconfig_server.json

          # Restart sing-box service
          pkill sing-box
          ./sbox/sing-box run -c ./sbox/sbconfig_server.json &
      ...
      5)
          ...
          if ./sbox/sing-box check -c ./sbox/sbconfig_server.json; then
...
echo "自动生成基本参数"
echo ""
key_pair=./sbox/sing-box generate reality-keypair
echo "Key pair生成完成"
echo ""
...
echo "$public_key" | base64 > ./sbox/public.key.b64

# Generate necessary values
uuid=$(./sbox/sing-box generate uuid)
short_id=$(./sbox/sing-box generate rand --hex 8)
echo "uuid和短id 生成完成"
echo ""
# Ask for listen port
...
hy_password=$(./sbox/sing-box generate rand --hex 8)
...
mkdir -p ./self-cert/ && openssl ecparam -genkey -name prime256v1 -out ./self-cert/private.key && openssl req -new -x509 -days 36500 -key ./self-cert/private.key -out ./self-cert/cert.pem -subj "/CN=${hy_server_name}"
echo ""
echo "自签证书生成完成"
echo ""
# vmess ws
  ...
  "inbounds": [
    ...
    },
    {
        ...
        "tls": {
            ...
            "certificate_path": "./self-cert/cert.pem",
            "key_path": "./self-cert/private.key"
        }
    },
    {
        "type": "vmess",
